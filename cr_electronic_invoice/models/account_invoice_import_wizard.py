...
class ImportInvoiceImportWizardCR(models.TransientModel):
    ...
    static_product_id = fields.Many2one('product.product',
                                        string='Producto a asignar en las líneas',
    ...
    def _create_invoice_from_file(self, attachment):
        ...
        try:
            invoice_xml = etree.fromstring(
                attachment.index_content.encode('UTF-8'))
        ...
        invoice.xml_supplier_approval = base64.encodestring(
            attachment.index_content.encode('UTF-8'))
        api_facturae.load_xml_data(invoice, True, self.account_id.id,
                                   self.static_product_id.id,
