...
class AccountInvoiceElectronic( models.Model ):
    ...
    def _onchange_xml_supplier_approval(self):
        if self.xml_supplier_approval:
            ...
            if not (root.findall( 'ResumenFactura' ) and root.findall( 'ResumenFactura' )[0].findall(
                    'TotalImpuesto' )):
                return {'value': {'xml_supplier_approval': False}, 'warning': {'title': 'Atención',
    ...
    def charge_xml_data(self):
        if self.xml_supplier_approval:
            ...
            if partner:
                ...
                raise UserError(
                    'El proveedor con identificación ' + root.findall( 'Emisor' )[0].find( 'Identificacion' )[
            ...
            self.amount_total_electronic_invoice = root.findall( 'ResumenFactura' )[0].findall( 'TotalComprobante' )[
                0].text
    ...
    def send_xml(self):
        for inv in self:
            if inv.xml_supplier_approval:
                ...
                root = ET.fromstring(
                    re.sub( ' xmlns="[^"]+"', '', base64.b64decode( inv.xml_supplier_approval ), count=1 ) )
                ...
                if float(
                        root.findall( 'ResumenFactura' )[0].findall( 'TotalComprobante' )[0].text ) == inv.amount_total:
                    if inv.company_id.frm_ws_ambiente != 'disabled' and inv.state_invoice_partner:
                        ...
                        payload['monto_total_impuesto'] = \
                        root.findall( 'ResumenFactura' )[0].findall( 'TotalImpuesto' )[
                            0].text
                        payload['total_factura'] = root.findall( 'ResumenFactura' )[0].findall( 'TotalComprobante' )[
                            0].text
        ...
        for i in invoices:
            ...
            if i.number_electronic and len( i.number_electronic ) == 50:
                ...
                if estado_m_h == 'aceptado':
                    ...
                    if not i.partner_id.opt_out and i.type == 'out_invoice':
                            ...
                            for i in inv_line.invoice_line_tax_ids:
                                ...
                                if i.tax_code != '00':
                                    ...
                                    impuestos = (impuestos + '"' + str( indextax ) + '":' + '{"codigo": "'
                                                 + str( i.tax_code or '01' ) + '",' + '"tarifa": "' + str(
                                                i.amount ) + '",' +
